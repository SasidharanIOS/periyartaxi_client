...
function BarVis({ pct, accent, accentBg, delay }) {
  return (
    <motion.div initial={{ height: 0 }} animate={{ height: `${pct}%` }}
      ...
      <div className="absolute top-0 left-0 right-0 h-[3px] rounded-t-lg" style={{ background: accent }} />
    </motion.div>
  );
}

/* ── Shared Revenue Streams List ── */
    ...
    <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
      {revenues.map((r, i) => (
        <motion.div key={i}
          ...
          style={{ minHeight: 0, borderBottom: i < revenues.length - 1 ? `1px solid ${LT.divider}` : "none", padding: compact ? "0 10px" : "0 16px" }}
          onMouseEnter={e => e.currentTarget.style.background = `${r.accent}06`}
          onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
          <div className="absolute left-0 top-0 bottom-0 w-[3px] opacity-0 transition-opacity duration-150"
          ...
          <div className="min-w-0 mr-2">
            ...
            {!compact && (
              <div className="font-body leading-tight"
                style={{ fontSize: "clamp(9px,1vw,12px)", color: LT.textFaint }}>{r.note}</div>
            )}
          </div>
          <span className="font-body font-bold flex-shrink-0 px-2 py-0.5 rounded-full"
            style={{ fontSize: compact ? "clamp(10px,2.8vw,13px)" : "clamp(12px,1.35vw,18px)", color: r.accent, background: r.accentBg, border: `1px solid ${r.accent}28` }}>
            {r.val}
          </span>
        </motion.div>
      ))}
      ...
      <motion.div variants={container} initial="hidden" animate="show"
        ...
        <motion.div variants={item} className="flex-shrink-0">
          ...
          <h2 className="font-display leading-none mt-0.5"
            ...
            REVENUE{" "}
            <span style={{ WebkitTextStroke: `2px ${LT.amber}`, color: "transparent" }}>MODEL</span>
          </h2>
          ...
          <motion.div variants={item}
            ...
            <div className="px-4 py-2.5 flex-shrink-0 flex items-center gap-2 relative"
              ...
              <span className="font-body font-bold"
                style={{ fontSize: "clamp(13px,1.5vw,19px)", color: LT.text }}>Revenue Streams</span>
            </div>
          ...
          <div className="flex flex-col gap-3 min-h-0">
            ...
            <motion.div variants={item}
              ...
              <div className="flex items-center gap-2 mb-3 mt-1">
                ...
                <span className="font-body font-bold"
                  style={{ fontSize: "clamp(13px,1.5vw,19px)", color: LT.text }}>3-Year Revenue Projection</span>
              </div>
              <div className="flex items-end justify-around gap-4" style={{ height: 120 }}>
                {projections.map((p, i) => (
                  <div key={i} className="flex-1 flex flex-col items-center gap-1">
                    <span className="font-body font-bold"
                      style={{ fontSize: "clamp(11px,1.2vw,16px)", color: p.accent }}>{p.rev}</span>
                    <div className="w-full flex flex-col justify-end" style={{ height: 80 }}>
                      <BarVis accent={p.accent} accentBg={p.accentBg} pct={p.barPct} delay={0.4 + i * 0.15} />
                    </div>
                    <span className="font-body font-semibold"
                      style={{ fontSize: "clamp(10px,1.1vw,14px)", color: LT.textMuted }}>{p.year}</span>
                  </div>
                ))}
              </div>
            </motion.div>
              ...
              <div className="grid grid-cols-4 px-4 py-2.5 flex-shrink-0 text-center"
                ...
                {["Year","Rides/Yr","Revenue","Drivers"].map((h, i) => (
                  <span key={h} className="font-body font-bold uppercase tracking-wide"
                    style={{ fontSize: "clamp(10px,1.2vw,15px)", color: i === 0 ? LT.amber : LT.textMuted }}>{h}</span>
                ))}
              </div>
              <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
                {projections.map((p, i) => (
                  <motion.div key={i} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                    ...
                    <span className="font-body font-bold px-2 py-0.5 rounded-full inline-block"
                      style={{ fontSize: "clamp(12px,1.3vw,17px)", color: p.accent, background: p.accentBg }}>{p.year}</span>
                    <span className="font-body tabular-nums"
                      style={{ fontSize: "clamp(11px,1.2vw,15px)", color: LT.textMuted }}>{p.rides.toLocaleString()}</span>
                    <span className="font-body font-bold"
                      style={{ fontSize: "clamp(12px,1.3vw,17px)", color: "#16a34a" }}>{p.rev}</span>
                    <span className="font-body tabular-nums"
                      style={{ fontSize: "clamp(11px,1.2vw,15px)", color: LT.textMuted }}>{p.drivers}</span>
                  </motion.div>
                ))}
              </div>
            </motion.div>
        ...
        <div className="flex md:hidden flex-1 flex-col min-h-0" style={{ gap: "clamp(6px,2vw,10px)" }}>
          ...
          <motion.div variants={item}
            ...
            style={{ background: LT.surface, border: `1.5px solid ${LT.amber}22`, boxShadow: LT.shadowMd, maxHeight: "38%" }}>
            <div className="px-3 py-2 flex items-center gap-2 flex-shrink-0 relative"
              ...
              <span className="font-body font-bold" style={{ fontSize: 12, color: LT.text }}>Revenue Streams</span>
            </div>
            <RevenueList compact={true} />
          </motion.div>
          ...
          <motion.div variants={item}
            ...
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="font-body font-bold" style={{ fontSize: 11, color: LT.text }}>3-Year Projection</span>
            </div>
            <div className="flex items-end justify-around gap-3" style={{ height: 70 }}>
              {projections.map((p, i) => (
                <div key={i} className="flex-1 flex flex-col items-center gap-0.5">
                  <span className="font-body font-bold" style={{ fontSize: "clamp(9px,2.8vw,12px)", color: p.accent }}>{p.rev}</span>
                  <div className="w-full flex flex-col justify-end" style={{ height: 48 }}>
                    <BarVis accent={p.accent} accentBg={p.accentBg} pct={p.barPct} delay={0.4 + i * 0.15} />
                  </div>
                  <span className="font-body font-semibold" style={{ fontSize: "clamp(8px,2.5vw,11px)", color: LT.textMuted }}>{p.year}</span>
                </div>
              ))}
            </div>
          </motion.div>

          {/* Projection table — compact, 3 cols (drop Drivers) */}
          <motion.div variants={item}
            ...
            <div className="grid grid-cols-3 px-3 py-1.5 text-center flex-shrink-0"
              ...
              {["Year","Rides/Yr","Revenue"].map((h, i) => (
                <span key={h} className="font-body font-bold uppercase tracking-wide"
                  style={{ fontSize: 9, color: i === 0 ? LT.amber : LT.textMuted }}>{h}</span>
              ))}
            </div>
            {projections.map((p, i) => (
              <div key={i} className="grid grid-cols-3 px-3 py-2 text-center items-center"
                style={{ borderBottom: i < projections.length - 1 ? `1px solid ${LT.divider}` : "none" }}>
                <span className="font-body font-bold px-2 py-0.5 rounded-full inline-block"
                  style={{ fontSize: 11, color: p.accent, background: p.accentBg }}>{p.year}</span>
                <span className="font-body tabular-nums"
                  style={{ fontSize: 11, color: LT.textMuted }}>{p.rides.toLocaleString()}</span>
                <span className="font-body font-bold"
                  style={{ fontSize: 11, color: "#16a34a" }}>{p.rev}</span>
              </div>
            ))}
          </motion.div>
...
function TotalCallout({ totalRev }) {
  return (
    <motion.div
      ...
      style={{ background: LT.surface, border: `1.5px solid ${LT.amber}35`, boxShadow: `0 4px 20px ${LT.amber}18`, padding: "clamp(8px,1.2vw,14px) clamp(12px,1.8vw,20px)" }}>
