...
/* ── Comparison table — shared between layouts ── */
  ...
  return (
    <div className="rounded-2xl overflow-hidden flex flex-col h-full"
      ...
      <div className="grid grid-cols-3 flex-shrink-0"
        style={{ background: `linear-gradient(90deg, ${LT.amber}12, transparent)`, borderBottom: `1.5px solid ${LT.divider}`, padding: compact ? "8px 12px" : "12px 16px" }}>
        <div className="font-body font-bold uppercase tracking-wider"
          style={{ fontSize: "clamp(9px,1vw,13px)", color: LT.textFaint }}>Feature</div>
        <div className="flex items-center justify-center gap-1.5">
          <div className="w-5 h-5 rounded-lg flex items-center justify-center flex-shrink-0"
            style={{ background: LT.amberBg, color: LT.amber }}>
            <FaTaxi size={11} />
          </div>
          <span className="font-body font-bold" style={{ fontSize: "clamp(11px,1.2vw,16px)", color: LT.amber }}>
        ...
        <div className="flex items-center justify-center gap-1.5">
          ...
          <span className="font-body font-bold" style={{ fontSize: "clamp(11px,1.2vw,16px)", color: "#dc2626" }}>Red Taxi</span>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
        {comparison.map((row, i) => (
          <motion.div key={i}
            ...
            style={{ minHeight: 0, borderBottom: i < comparison.length - 1 ? `1px solid ${LT.divider}` : "none", padding: compact ? "0 12px" : "0 16px" }}
            onMouseEnter={e => e.currentTarget.style.background = `${LT.amber}07`}
            onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
            <span className="font-body leading-tight pr-2"
              style={{ fontSize: compact ? "clamp(9px,2.5vw,12px)" : "clamp(11px,1.2vw,15px)", color: LT.text }}>
              {row.feature}
            </span>
      ...
      {/* Winner strip */}
      <div className="px-3 py-2 text-center flex-shrink-0 flex items-center justify-center gap-2"
        style={{ background: `linear-gradient(90deg, transparent, ${LT.amber}14, transparent)`, borderTop: `1.5px solid ${LT.amber}25` }}>
        <span className="font-body font-bold"
          style={{ fontSize: "clamp(10px,1.2vw,16px)", color: LT.amber }}>
          🏆 Periyar Taxi wins 7 / 8 categories vs Red Taxi
        </span>
      </div>
    </div>
  );
      ...
      <motion.div variants={container} initial="hidden" animate="show"
        ...
        <motion.div variants={item} className="flex-shrink-0">
          ...
          <h2 className="font-display leading-none mt-0.5"
            ...
            MARKET &amp;{" "}
            <span style={{ WebkitTextStroke: `2px ${LT.amber}`, color: "transparent" }}>COMPETITION</span>
          </h2>
        ...
        {/* ── Desktop layout: 2-col left + 3-col comparison ── */}
          ...
          <div className="flex flex-col gap-3 min-h-0" style={{ width: "38%" }}>
            ...
            <motion.div variants={item}
              ...
              <div className="flex items-center gap-2 mb-3 mt-1 flex-shrink-0">
                ...
                <span className="font-body font-bold" style={{ fontSize: "clamp(13px,1.4vw,18px)", color: LT.text }}>Indian Taxi Market</span>
              </div>
              <div className="flex-1 flex flex-col justify-between">
                {marketStats.map((s, i) => (
                  <div key={i} className="flex justify-between items-center py-1.5"
                    style={{ borderBottom: i < marketStats.length - 1 ? `1px solid ${LT.divider}` : "none" }}>
                    <span className="font-body" style={{ fontSize: "clamp(11px,1.1vw,14px)", color: LT.textMuted }}>{s.label}</span>
            ...
            <motion.div variants={item}
              ...
              <div className="flex items-center gap-2 mb-2 mt-1 flex-shrink-0">
                ...
                <span className="font-body font-bold" style={{ fontSize: "clamp(13px,1.4vw,18px)", color: LT.text }}>Our Reach</span>
              </div>
              <p className="font-body leading-snug mb-2"
                style={{ fontSize: "clamp(11px,1.1vw,14px)", color: LT.textMuted }}>
                Origin: <span style={{ color: LT.amber, fontWeight: 700 }}>Erode, Tamil Nadu</span><br />
                Service: <span style={{ color: "#2563eb", fontWeight: 700 }}>All India</span> — outstation, corporate, airport.
              </p>
        ...
        <div className="flex md:hidden flex-1 flex-col min-h-0" style={{ gap: "clamp(6px,2vw,10px)" }}>
          ...
          {/* Market stats — horizontal scrolling row */}
          <motion.div variants={item}
            ...
            <div className="flex items-center gap-2 mb-2 mt-0.5">
              ...
              <span className="font-body font-bold" style={{ fontSize: 12, color: LT.text }}>Indian Taxi Market</span>
            </div>
            ...
            <div className="grid grid-cols-2 gap-1.5">
              {marketStats.map((s, i) => (
                <div key={i} className="rounded-lg px-2.5 py-1.5 flex flex-col"
                  ...
                  <span className="font-body font-bold" style={{ fontSize: "clamp(14px,4vw,18px)", color: s.accent }}>{s.val}</span>
                  <span className="font-body" style={{ fontSize: "clamp(9px,2.5vw,11px)", color: LT.textMuted }}>{s.label}</span>
                </div>
              ))}
            </div>
          </motion.div>

          {/* Reach tags — compact inline */}
          <motion.div variants={item}
            className="flex-shrink-0 rounded-xl relative overflow-hidden"
            style={{ background: LT.surface, border: "1.5px solid rgba(37,99,235,0.18)", boxShadow: LT.shadowMd, padding: "8px 12px" }}>
            <div className="absolute top-0 left-0 right-0 h-[3px]" style={{ background: "#2563eb" }} />
            <div className="flex items-center gap-3 flex-wrap mt-0.5">
              <div className="flex items-center gap-1.5">
                <MdLocationOn size={14} style={{ color: "#2563eb" }} />
                <span className="font-body font-bold" style={{ fontSize: 11, color: LT.text }}>Our Reach:</span>
              </div>
              {reachTags.map((c, i) => (
                <span key={c} className="px-2 py-0.5 rounded-full font-body font-semibold"
                  style={{ fontSize: 10, background: tagBgs[i], border: `1px solid ${tagColors[i]}28`, color: tagColors[i] }}>
                  {c}
                </span>
              ))}
            </div>
          ...
          {/* Comparison table — takes remaining space */}
          <motion.div variants={item} className="flex-1 min-h-0">
