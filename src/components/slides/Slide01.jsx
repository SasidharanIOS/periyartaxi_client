...
const CARS = [
  { key: "tavera",  name: "Chevrolet Tavera",   emoji: "🚐", tag: "Premium MPV", color: "#3b82f6", img: `${import.meta.env.BASE_URL}chevorlet1.png` },
  { key: "swift",   name: "Maruti Swift Dzire",  emoji: "🚗", tag: "Sedan",       color: "#ef4444", img: `${import.meta.env.BASE_URL}dezire.png`     },
  { key: "aura",    name: "Hyundai Aura",        emoji: "🚘", tag: "Sedan",       color: "#a3a3a3", img: `${import.meta.env.BASE_URL}aura.jpg`        },
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-0 left-0 right-0 px-2 pb-1.5 flex items-end justify-between">
        ...
        <span className="px-1.5 py-0.5 rounded-full text-white font-body font-semibold flex-shrink-0 ml-1"
  ...
  return (
    <div className="w-full h-full bg-taxi-yellow relative overflow-hidden flex flex-col">
      ...
      <motion.div
        ...
        className="relative z-10 flex flex-col w-full h-full px-3 pt-2 pb-12 gap-2"
      >
        {/* Logo */}
        ...
        {/* 2×2 Car Grid — on mobile uses full flex-1, on desktop same */}
        <motion.div
          variants={itemVar}
            ...
            {STATS.map((s, i) => (
              <React.Fragment key={s.value}>
                <div className="text-center">
                  <div className="font-display text-taxi-black leading-none"
                    style={{ fontSize: "clamp(18px, 4vw, 52px)" }}>
                    {s.value}
                  </div>
                  <div className="font-body text-taxi-black/60 font-semibold uppercase tracking-wider"
